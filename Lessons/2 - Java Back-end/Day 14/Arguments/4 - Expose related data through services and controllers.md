...
> [!note] Pro tip
> The `nullable = false` from [[2 - Map employees to departments|second doc]] enforce the `Employee` to have a valid department id. Make sure this will checked and eventually reported to caller, instead of leaving it breaking the store process.
